<?php 
// Include the header
$pageTitle = isset($_GET['genre']) ? htmlspecialchars(ucfirst($_GET['genre'])) : "Movies";
include 'header.php';
...
foreach ($files as $file) {
    ...
    // Check if json_decode succeeded and if 'Genre' exists
    if (is_array($movieData) && isset($movieData['Genre']) && is_array($movieData['Genre'])) {
        if (in_array(ucfirst($genre), $movieData['Genre'])) { // Match the genre case
            $filteredFiles[] = $file;
        }
    }
}
...
$totalMovies = count($filteredFiles);
$totalPages = ceil($totalMovies / $moviesPerPage);

// Calculate starting index for pagination
$startIndex = ($page - 1) * $moviesPerPage;
$paginatedFiles = array_slice($filteredFiles, $startIndex, $moviesPerPage);
?>
    ...
    <div class="container">
        ...
        <div class="pagination-style mt-30">
            <ul>
                ...
                ?>
                        <li><a <?php echo ($i === $page) ? 'class="active"' : ''; ?> href="action.php?page=<?php echo $i; ?>&genre=<?php echo urlencode($genre); ?>"><?php echo $i; ?></a></li>
                <?php
                    endfor;
                endif;
                ?>
